...
/* function prototypes */
void Free3dDouble(double ***array, int size1, int size2);
...
FILE * fp_out_1;
fp_out_1 = file_open(flag_par_init.file_out,"w+");
...
for (int i = 0; i < num_checkpoints; i++){
	...
	double percent_traj_to_use = checkpoints[i];
      //FILE *fp_in_1, *fp_in_2, *fp_out_1; //Original code
      FILE *fp_in_2; //Yao 20230928
...
/* if wanted write out superimposed models */
     if(flag_par.wp)
     {
     //fp_out_1=fopen(flag_par.pdb_outfile,"w"); //Yao 20230928 Now only opening at the very beginning of int main().
     write_PDB_atoms(fp_out_1, system.n_atoms, system.atoms, trj);
     //fclose(fp_out_1); //Yao 20230928 Now only closing at the very beginning of int main().
     }
}
...
printf("Using the first %.0f%% of trajectory\n", percentage_value);
fprintf(fp_out_1,"Begin Entropy Calculation using the first %.0f%% of the entire trajectory\n", percentage_value);
for(k = 1; k <= flag_par.n; k++)
{
...
/* //Yao: I don't need this right now.
...
fprintf(fp_out_1,"Done Entropy Calculation using the first %.0f%% of the entire trajectory\n\n", percentage_value);
...
//Whatever I'm not freeing here gives memory error when I do so. I'm assuming they've been freed somewhere above. 
free(ent_k_tr); free(d_mean_tr); free(ld_mean_tr); free(ent_k_tr2);
free(ent_k_t); free(d_mean_t); free(ld_mean_t); free(ent_k_t2);
free(ent_k_r); free(d_mean_r); free(ld_mean_r); free(ent_k_r2); free(w);
Free2dDouble(R, 3); free(t); Free3dDouble(Ra, trj.nf, 3); Free2dDouble(ta, trj.nf); 
free(a); free(sd); free(x); free(y);
FreeTrajectory(&trj);
...
for(i = 0; i< n; i++)
{
//printf("In fit 1w x[%d] and w[%d]: %f, %f \n", i, i, x[i], w[i]);
xm = xm + x[i]*w[i];
ym = ym + y[i]*w[i];
y2 = y2 + y[i]*y[i]*w[i];
x2 = x2 + x[i]*x[i]*w[i];
...
xy = xy / wt;
xy2 = xy2 / wt;
//printf("In fit 1w xm and x2: %f, %f \n", xm, x2);
//printf("In fit 1w a[1] top and bottom: %f \n", (x2 - xm*xm));
a[1] = (xy - xm * ym)/(x2 - xm*xm);
a[0] = ym - a[1] * xm ;
//printf ("In fit1w a 1 and 0: %f, %f \n", a[1], a[0]);
sig2 = 0;
for(i = 0; i< n; i++)
