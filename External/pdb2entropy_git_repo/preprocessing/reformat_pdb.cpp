...
int main(int argc, char* argv[])
{
    ...
    for (auto& ligandResidue : ligandResidues) // Each MODEL in PdbFile is converted into an "Assembly"
    {
		...
		std::vector<cds::Atom*> atoms = ligandResidue->getAtoms();
		int index = 1; //Temporary
		std::cout << "Num atoms: " << atoms.size() << std::endl;
		for (auto& a : atoms){
			std::stringstream name;
			name << static_cast<pdb::PdbAtom*>(a)->GetElementSymbol();
			//name << a->getElement();
			name << index;
			//std::cout << "Old vs new name: " << a->getName() << " vs " << name.str() << std::endl;
			//a->setName(name.str());
			index++;
		}
    }
