...
double wt_gau1 = -0.035579, wt_gau2 = -0.005156, wt_rep = 0.840245, wt_pho = -0.035069, wt_hb = -0.587439, wt_catpi = -0.40, wt_pi_pi = -0.00;
//double wt_ch_pi = -0.600000;
double wt_ch_pi = 0.00;
        ...
        for (unsigned int rec_index = 0; rec_index < receptor_atoms_valid_types_no_H.size(); rec_index++){
	    	...
	    	if (distance_within_cutoff){
                ...
                if (element_xs_vdw_radii.find(lig_xs_type) != element_xs_vdw_radii.end() && element_xs_vdw_radii.find(rec_xs_type) != element_xs_vdw_radii.end()){
					...
					//std::cout << "Lig xs type & radius: " << lig_xs_type << " - " << lig_atm_xs_radius << std::endl;
					//std::cout << "Rec xs type & radius: " << rec_xs_type << " - " << rec_atm_xs_radius << std::endl;
                }
                else {
                ...
                if (rep_distance < 0){
                    tot_rep += rep_distance * rep_distance;
                    //std::cout << "Rep between " << lig_atm->GetName() << " and " << rec_atm->GetResidue()->GetName() << "-" << rec_atm->GetName()  << " is: rep dist: " << -rep_distance  << " and " << (wt_rep * rep_distance * rep_distance) << std::endl;
                }
