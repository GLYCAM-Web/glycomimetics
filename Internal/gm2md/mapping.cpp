...
void BuildBondFromMol2(MolecularModeling::Assembly& ligand_pdb, std::vector<std::pair<int, int>>& mol2_bonding_indices, std::string resp_charge_output_file_path){
    ...
    for (unsigned int i = 0; i < all_atoms.size(); i++){
        ...
        //std::cout << " Ligand pdb atom: "<< i+1 << "-" << all_atoms[i]->GetName() << std::endl;
...
void ParseMol2File(char* path, std::vector<mol2_atom>& mol2_atoms, std::vector<std::pair<int, int>>& mol2_bonding_indices){
    ...
    while (mol2 >> id >> atom1_index >> atom2_index >> bond_type){
		...
		//std::cout << "Mol2 bonding index pair: " << atom1_index << " -  " << atom2_index << std::endl;
...
bool type_matches(std::string type1, std::string type2, bool strict){
    gmml::Trim(type1);
    gmml::Trim(type2);
...
bool torsion_matches(torsion& glycam_gaff_torsion, TorsionBlock* std_torsion, AtomVector& pdb_atoms, std::vector<mol2_atom>& mol2_atoms, std::vector<TorsionBlock*>& interface_torsions, bool improper = false){
    ...
    if (improper){
        ...
        for (unsigned int i = 0; i < arrangements_of_four.size(); i++){
	    	...
	    	if (type_matches(gaff_type1, template_types[arrangement[0]], true) && type_matches(gaff_type2, template_types[arrangement[1]], true) && type_matches(gaff_type3, template_types[arrangement[2]], true) &&	type_matches(gaff_type4, template_types[arrangement[3]], true)){
				match = true;
			}
	    	else if (type_matches(gaff_type1, template_types[arrangement[0]], false) && type_matches(gaff_type2, template_types[arrangement[1]], false) && type_matches(gaff_type3, template_types[arrangement[2]], false) &&	type_matches(gaff_type4, template_types[arrangement[3]], false)){
				match = true;
			}
    ...
    //If proper torsion
    else if ((type_matches(gaff_type1, ipt, true) && type_matches(gaff_type2, jpt, true) && type_matches(gaff_type3, kpt, true) && type_matches(gaff_type4, lpt, true)) || (type_matches(gaff_type1, lpt, true) && type_matches(gaff_type2, kpt, true) && type_matches(gaff_type3, jpt, true) && type_matches(gaff_type4, ipt, true))){
		match = true;
    }
    else if ((type_matches(gaff_type1, ipt, false) && type_matches(gaff_type2, jpt, false) && type_matches(gaff_type3, kpt, false) && type_matches(gaff_type4, lpt,false)) || (type_matches(gaff_type1, lpt, false) && type_matches(gaff_type2, kpt, false) && type_matches(gaff_type3, jpt, false) && type_matches(gaff_type4, ipt, false))){
		match = true;
    }

    if (match){
        TorsionBlock* new_torsion_block = new TorsionBlock(atom1->MolecularDynamicAtom::GetAtomType(), atom2->MolecularDynamicAtom::GetAtomType(), atom3->MolecularDynamicAtom::GetAtomType(), atom4->MolecularDynamicAtom::GetAtomType(), "", "", "", "", std_torsion->COMMENT);
...
void MapInterface2GaffParm(std::vector<std::pair<MolecularModeling::Atom*, MolecularModeling::Atom*>>& glycam_gaff_bonds, std::vector<angle>& glycam_gaff_angles, std::vector<torsion>& glycam_gaff_torsions, std::vector<torsion>& glycam_gaff_improper_torsions, std::vector<BondLine*>& interface_bond_lines, std::vector<AngleLine*>& interface_angle_lines, std::vector<TorsionBlock*>& interface_torsions, std::vector<TorsionBlock*>& interface_improper_torsions, SixTwelvePotential* interface_nonbon, MainParm& amber_gaff_dat, Frcmod& antechamber_frcmod, AtomVector& pdb_atoms, std::vector<mol2_atom>& mol2_atoms){
    ...
    for (unsigned int i = 0; i < glycam_gaff_bonds.size(); i++){
		...
		for (unsigned int j = 0; j < std_gaff_bonds.size(); j++){
			...
			bool match = false;
	    	if ((type_matches(gaff_type1, ibt, true) && type_matches(gaff_type2, jbt, true)) || (type_matches(gaff_type1, jbt, true) && type_matches(gaff_type2,ibt, true))){
				match = true;
			}
	    	else if ((type_matches(gaff_type1, ibt, false) && type_matches(gaff_type2, jbt, false)) || (type_matches(gaff_type1, jbt, false) && type_matches(gaff_type2,ibt, false))){
				match = true;
			}
    ...
    for (unsigned int i = 0; i < glycam_gaff_angles.size(); i++){
		...
		for (unsigned int j = 0; j < std_gaff_angles.size(); j++){
	    	...
	    	if ((type_matches(gaff_type1, itt, true) && type_matches(gaff_type2, jtt, true) && type_matches(gaff_type3, ktt, true)) || (type_matches(gaff_type1, ktt, true) && type_matches(gaff_type2, jtt, true) && type_matches(gaff_type3, itt, true))){
				match = true;
			}
	    	else if ((type_matches(gaff_type1, itt, false) && type_matches(gaff_type2, jtt, false) && type_matches(gaff_type3, ktt, false)) || (type_matches(gaff_type1, ktt, false) && type_matches(gaff_type2, jtt, false) && type_matches(gaff_type3, itt, false))){
				match = true;
			}
			...
			if (match){
	        	...
	        	interface_angle_lines.emplace_back(new AngleLine(atom1->MolecularDynamicAtom::GetAtomType(), atom2->MolecularDynamicAtom::GetAtomType(), atom3->MolecularDynamicAtom::GetAtomType(), std_angle->TK, std_angle->TEQ, comment.str()));
				break;
			}
	    }
	}
