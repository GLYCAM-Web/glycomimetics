...
#include "../gm/vina_bond_by_distance_for_pdb.hpp"
...
{
	int num_args_expected = 9;
	int num_args_given = argc -1;
	if (num_args_given != num_args_expected){
		std::cout << "Glycam gaff interfacing: Expect " << num_args_expected << " arguments, " << num_args_given << " given. " << std::endl;
		std::cout << "Usage: ./main.exe mol2 ligand_pdb pdb2glycam_log gaff_dat corona.frcmod resp_out('none' if not given) output_frcmod output_off net_charge" << std::endl; 
		std::exit(1);
	}
